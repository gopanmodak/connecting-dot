...
import { StarIcon, Heart, Plus, Minus, ChevronLeft, ChevronRight, MessageSquare } from "lucide-react";
...
const ProductDetails = () => {
  ...
  if (!product) {
    return <h2 className="text-center text-white py-20 bg-[#0f0117]">Product not found</h2>;
  }
        ...
        return (
          <div className="text-gray-400 space-y-6 animate-in fade-in duration-500">
            <div className="bg-[#1a012a] p-4 rounded-lg border border-gray-800">
              <p className="text-white font-semibold">Q: How many devices can I use?</p>
              <p className="text-sm mt-2">A: This is a private screen, so you can use it on one device at a time.</p>
            </div>
            <div className="bg-[#1a012a] p-4 rounded-lg border border-gray-800">
              <p className="text-white font-semibold">Q: Is it a full month warranty?</p>
              <p className="text-sm mt-2">A: Yes, we provide a full 30-day replacement warranty.</p>
            </div>
          </div>
        );
      case "Reviews":
          ...
          <div className="space-y-12 animate-in fade-in duration-500">
            ...
            <div className="flex gap-2 mb-8">
              ...
              <div className="flex text-yellow-400">
                ...
                {[...Array(4)].map((_, i) => <StarIcon key={i} size={14} fill="currentColor" />)}
              ...
              {[1, 2].map((review) => (
                <div key={review} className="flex gap-5 border-t border-gray-900 pt-8">
                  <div className="w-12 h-12 rounded-full bg-gray-800 overflow-hidden shrink-0">
                    <img src={`https://i.pravatar.cc/150?u=${review}`} alt="user" />
                  </div>
                  <div className="space-y-2">
                    <h4 className="font-bold text-white">{review === 1 ? "Sofia Harvetz" : "Nicolas Jensen"}</h4>
                    <div className="flex text-yellow-400">
                      {[...Array(5)].map((_, i) => <StarIcon key={i} size={12} fill="currentColor" />)}
                    </div>
                    ...
                    <div className="flex gap-4 text-[10px] text-gray-500 mt-4">
                      ...
                      <button className="hover:text-white flex items-center gap-1"><MessageSquare size={10}/> Reply</button>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className="relative group">
            <div className="bg-[#1a012a] rounded-2xl p-8 aspect-square flex flex-col items-center justify-center border border-gray-800 shadow-2xl relative overflow-hidden">
              ...
              <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end opacity-50 text-[10px] font-mono">
                <div>
                  <p>+880 016 0454 1255</p>
                  <p>www.fanflixbd.com</p>
                </div>
                <img src="/sword-icon.png" alt="" className="h-6 w-auto rotate-45" />
              </div>
          ...
          <div className="space-y-6">
            ...
            <p className="text-sm text-gray-400 leading-relaxed max-w-md">
                {product.description || "Buy one or buy a few and make every space where you sit more convenient. Light and easy to move around."}
            </p>
